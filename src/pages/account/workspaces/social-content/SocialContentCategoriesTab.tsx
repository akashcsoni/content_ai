...
import { ApiError, socialContentApi, type CategoryConcept } from '../../../../lib/api'
import { useAuth } from '../../../../context/AuthContext'
import PromptShortcodeReference from '../auto-blog/PromptShortcodeReference'
import {
  DEFAULT_SOCIAL_CATEGORY_PROMPT,
  type SocialContentCategory,
  type SocialContentSettings,
} from './socialContent.types'

type SocialContentCategoriesTabProps = {
...
export default function SocialContentCategoriesTab({ settings }: SocialContentCategoriesTabProps) {
  const { token } = useAuth()
  ...
  const loadCategories = useCallback(async () => {
    ...
    try {
      const response = await socialContentApi.listCategories(token)
  ...
  useEffect(() => {
    async function loadConcepts() {
      ...
      try {
        const response = await socialContentApi.listCategoryConcepts(token)
  ...
  async function handleSaveCategory(categoryId: string) {
    ...
    try {
      await socialContentApi.updateCategory(token, categoryId, {
  ...
  async function handleDeleteCategory(categoryId: string) {
    ...
    setError('')
    try {
      await socialContentApi.deleteCategory(token, categoryId)
      await loadCategories()
  ...
  async function handleCreateCategory() {
    ...
    try {
      await socialContentApi.createCategory(token, {
  ...
  async function handleImportConcept() {
    ...
    try {
      const response = await socialContentApi.importCategoryConcept(token, {
  ...
  async function handleGenerateTopics(categoryId: string) {
    ...
    try {
      const response = await socialContentApi.generateTopics(token, { count: 10, categoryId })
      await loadCategories()
      flashSuccess(response.message)
    } catch (err) {
  ...
  function renderCategoryEditor(category: SocialContentCategory) {
        ...
        <div className="service-settings-section-body">
          <div className="service-settings-field-group">
            ...
            <div className="service-settings-grid">
              <label htmlFor={`social-category-name-${category.id}`}>
                Name
                <input
                  id={`social-category-name-${category.id}`}
                  type="text"
                  value={draft.name}
              ...
              <label htmlFor={`social-category-description-${category.id}`}>
                Description
                <input
                  id={`social-category-description-${category.id}`}
                  type="text"
          ...
          <div className="service-settings-field-group">
            ...
            <label htmlFor={`social-category-prompt-${category.id}`}>
              Prompt instructions
              <textarea
                id={`social-category-prompt-${category.id}`}
                rows={5}
                value={draft.prompt}
                onChange={(event) => updateDraft(category.id, { prompt: event.target.value })}
                placeholder={DEFAULT_SOCIAL_CATEGORY_PROMPT}
              />
            ...
            <div className="service-settings-actions service-settings-actions--inline service-settings-actions--group">
              <button
                ...
                onClick={() => updateDraft(category.id, { prompt: DEFAULT_SOCIAL_CATEGORY_PROMPT })}
              >
    ...
    <div className="service-tab-panel">
      <div className="service-tab-toolbar">
        <div>
          <h2>Categories</h2>
          <p>
            Each category has its own content prompt. Topics and social posts use that prompt when
      ...
      {settings && !settings.enabled && (
        <p className="service-workspace-alert service-workspace-alert--warning">
          Social content is inactive. Activate it on the Topic queue tab to generate category topics
          or import packs.
        </p>
      )}
      ...
      <div className="service-settings-accordion service-settings-accordion--categories">
        <details className="service-settings-section">
          ...
          <div className="service-settings-section-body">
            <div className="service-settings-field-group">
              <p className="service-settings-field-group-title">Brand concept</p>
              <p className="service-field-hint service-category-import-hint">
                AI creates 10 categories with prompts using your brand voice and language from
                Settings.
              </p>
        ...
        <details className="service-settings-section service-settings-section--add">
          ...
          <div className="service-settings-section-body">
            <div className="service-settings-field-group">
              <p className="service-settings-field-group-title">Category details</p>
              <div className="service-settings-grid">
                <label htmlFor="social-new-category-name">
                  Name
                  <input
                    id="social-new-category-name"
                    type="text"
                ...
                <label htmlFor="social-new-category-description">
                  Description
                  <input
                    id="social-new-category-description"
                    type="text"
            ...
            <div className="service-settings-field-group">
              <p className="service-settings-field-group-title">Content prompt</p>
              <label htmlFor="social-new-category-prompt">
                Prompt instructions
                <textarea
                  id="social-new-category-prompt"
                  rows={4}
                  value={newCategory.prompt}
                  onChange={(event) =>
                    setNewCategory((current) => ({ ...current, prompt: event.target.value }))
                  }
                  placeholder={DEFAULT_SOCIAL_CATEGORY_PROMPT}
                />
                <span className="service-field-hint">
              ...
              <div className="service-settings-actions service-settings-actions--inline service-settings-actions--group">
                <button
                  ...
                  className="btn btn-secondary"
                  onClick={() =>
                    setNewCategory((current) => ({ ...current, prompt: DEFAULT_SOCIAL_CATEGORY_PROMPT }))
                  }
                >
                  Use default prompt
                </button>
