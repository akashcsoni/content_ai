...
import { ApiError, emailNewsletterApi, type CategoryConcept } from '../../../../lib/api'
import { useAuth } from '../../../../context/AuthContext'
import type { EmailNewsletterCategory, EmailNewsletterSettings } from './emailNewsletter.types'
import { DEFAULT_EMAIL_NEWSLETTER_CATEGORY_PROMPT } from './emailNewsletter.types'
import PromptShortcodeReference from './PromptShortcodeReference'

type EmailNewsletterCategoriesTabProps = {
...
export default function EmailNewsletterCategoriesTab({ settings }: EmailNewsletterCategoriesTabProps) {
  const { token } = useAuth()
  ...
  const loadCategories = useCallback(async () => {
    ...
    try {
      const response = await emailNewsletterApi.listCategories(token)
  ...
  useEffect(() => {
    async function loadConcepts() {
      ...
      try {
        const response = await emailNewsletterApi.listCategoryConcepts(token)
  ...
  async function handleSaveCategory(categoryId: string) {
    ...
    try {
      await emailNewsletterApi.updateCategory(token, categoryId, {
  ...
  async function handleDeleteCategory(categoryId: string) {
    ...
    setError('')
    try {
      await emailNewsletterApi.deleteCategory(token, categoryId)
      await loadCategories()
  ...
  async function handleCreateCategory() {
    ...
    try {
      await emailNewsletterApi.createCategory(token, {
  ...
  async function handleImportConcept() {
    ...
    try {
      const response = await emailNewsletterApi.importCategoryConcept(token, {
  ...
  async function handleGenerateTopics(categoryId: string) {
    ...
    try {
      const response = await emailNewsletterApi.generateTopics(token, { count: 10, categoryId })
      await loadCategories()
      flashSuccess(response.message)
    } catch (err) {
  ...
  function renderCategoryEditor(category: EmailNewsletterCategory) {
          ...
          <div className="service-settings-field-group">
            ...
            <label htmlFor={`category-prompt-${category.id}`}>
              ...
              <textarea
                ...
                placeholder={DEFAULT_EMAIL_NEWSLETTER_CATEGORY_PROMPT}
              />
            ...
            <div className="service-settings-actions service-settings-actions--inline service-settings-actions--group">
              <button
                ...
                onClick={() => updateDraft(category.id, { prompt: DEFAULT_EMAIL_NEWSLETTER_CATEGORY_PROMPT })}
              >
    ...
    <div className="service-tab-panel">
      <div className="service-tab-toolbar">
        <div>
          <h2>Categories</h2>
          <p>
            Each category has its own content prompt. Topics and newsletters use that prompt when
      ...
      {settings && !settings.enabled && (
        <p className="service-workspace-alert service-workspace-alert--warning">
          Email newsletters is inactive. Activate it on the Topic queue tab to generate category topics or import packs.
        </p>
      )}
            ...
            <div className="service-settings-field-group">
              ...
              <div className="service-settings-actions service-settings-actions--inline service-settings-actions--group">
                <button
                  ...
                  className="btn btn-secondary"
                  onClick={() =>
                    setNewCategory((current) => ({ ...current, prompt: DEFAULT_EMAIL_NEWSLETTER_CATEGORY_PROMPT }))
                  }
                >
                  Use default prompt
                </button>
